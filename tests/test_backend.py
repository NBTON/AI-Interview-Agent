...
class TestBackend:
    ...
    def test_get_dashboard_stats(self, client):
        ...
        # Safe checks - handle None values
        if data["total_candidates"] is not None:
            assert data["total_candidates"] >= 0
        if data["completed_interviews"] is not None:
            assert data["completed_interviews"] >= 0
        if data["pending_interviews"] is not None:
            assert data["pending_interviews"] >= 0
        if data["in_progress_interviews"] is not None:
